...
MJScore::MJScore() {
    ...
    Jikaze = Bakaze = Honba = KyoutakuRiichi = ErrorCode = Haino = TokutenKo =
    TokutenOya = Fuurosuu = dora = uradora = akadora = Agarihai = tmp_fu =
    result_fu = tmp_fan = result_fan = TokutenKoTumo = TokutenOyaTumo =
    inputteddora = // v1.03
    0;
    ...
    Akahai = Tenhou = Tiihou = Riichi = DoubleRiichi = Ippatu = Tyankan =
    Rinsyan = NagashiMangan = ManganKiriage = Tumo = Haitei = Ba1500 =
    DoubleKokushi13 = DoubleTyuuren9 = DoubleDaisuusii = DoubleSuttan =
    IsMangan = is_fuuro = false;
...
void MJScore::Clear() {
    ...
    Jikaze = Bakaze = Honba = KyoutakuRiichi = ErrorCode = Haino = TokutenKo =
    TokutenOya = TokutenKoTumo = TokutenOyaTumo = Fuurosuu = dora = uradora =
    akadora = Agarihai = tmp_fu = result_fu = tmp_fan = result_fan =
    inputteddora = // v1.03
    0;
...
void MJScore::Clear_WithoutRule() {
    ...
    Tenhou = Tiihou = Riichi = DoubleRiichi = Ippatu = Tyankan = Rinsyan =
    NagashiMangan = Tumo = Haitei = IsMangan = is_fuuro = false;
...
void MJScore::Tehai_Normalize() {
    ...
    for (int i = 0; Fuuro[i] && i <= 12; i += 4) {
        ...
        } else if ((Fuuro[i] == ANKAN) || (Fuuro[i] == MINKAN) ||
                   (Fuuro[i] == KAKAN)) {
            TehaiNormalizeMarge[Fuuro[i + 1]] += 3;
        }
    }
...
void MJScore::Decision_Score(const int fan, const int fu) {
    ...
    KoTumoOya =
    (KoTumoOya % 100 > 0) ? KoTumoOya + 100 - KoTumoOya % 100 : KoTumoOya;
...
void MJScore::MentuKiriwake() {
    ...
    for (i = 0; i < 20; i += 4) {
        if (Fuuro[i] != 0) {
            ...
            } else if (Fuuro[i] == PON || Fuuro[i] == AKA_PON1 ||
                       Fuuro[i] == AKA_PON2) {
    ...
    for (i = 0; i < 38; i++) {
        ...
        if (i >= 38) {
            if (kiriwake[9] != 0) { //４メンツ１頭とれてるなら
                ...
                Chk_Fu();
                //これまでで最高の役数＋符ならば記憶
                if ((tmp_fan > result_fan) ||
                    (tmp_fan == result_fan && tmp_fu > result_fu)) {
                    result_fan = tmp_fan;
                    result_fu = tmp_fu;
        ...
        //シュンツ抜き出し
        if (TehaiNormalize[i] && TehaiNormalize[i + 1] && TehaiNormalize[i + 2] &&
            i < 30) {
            TehaiNormalize[i]--, TehaiNormalize[i + 1]--, TehaiNormalize[i + 2]--;
            kiriwake[p_kiriwake] = SYUNTU, kiriwake[p_kiriwake + 1] = i,
            p_kiriwake += 2;
            KiriwakeNukidasi(); //再帰
            p_kiriwake -= 2, kiriwake[p_kiriwake] = 0, kiriwake[p_kiriwake + 1] = 0;
            TehaiNormalize[i]++, TehaiNormalize[i + 1]++, TehaiNormalize[i + 2]++;
...
void MJScore::Chk_Fu() {
    ...
    for (int i = 0; i < 10; i += 2) {
        if (kiriwake[i] == TOITU) {
            ...
            if ((kiriwake[i + 1] == Agarihai - 2 &&
                 kiriwake[i + 1] % 10 == 1) || //ペン３待ち
                (kiriwake[i + 1] == Agarihai &&
                 kiriwake[i + 1] % 10 == 7) ||     //ペン７待ち
    ...
    for (int i = 0; i < 10; i += 2) {
        if (kiriwake[i] == ANKO) { //暗コウ
            if (kiriwake[i + 1] % 10 == 1 || kiriwake[i + 1] % 10 == 9 ||
                kiriwake[i + 1] > 30)
                tmp_fu += 8; //ヤオチュウ
            else
                tmp_fu += 4;                 // 中張
...
bool MJScore::Is_Daisangen() {
    if (TehaiNormalizeMarge[35] == 3 && TehaiNormalizeMarge[36] == 3 &&
...
bool MJScore::Is_Tyuuren9() {
    int syurui = 0;
    int i;
    if ((bool)TehaiNormalizeMarge[1] + (bool)TehaiNormalizeMarge[11] +
        (bool)TehaiNormalizeMarge[21] !=
        1) // C言語でTRUEは数字の１として扱える
        return false;
    //字牌があれば当然FALSE
    for (i = 31; i <= 37; i++) {
        if (TehaiNormalizeMarge[i])
            return false;
    }
    //種類を記憶
    if (TehaiNormalizeMarge[1])
        syurui = 0;
    else if (TehaiNormalizeMarge[11])
        syurui = 1;
    else if (TehaiNormalizeMarge[21])
        syurui = 2;
...
bool MJScore::Is_Dora() {
    ...
    for (i = 1; i <= 5; i++) {
        if (Dora[i]) {
            ...
            for (int i = 0; Fuuro[i] && i <= 12; i += 4) {
                if (((Fuuro[i] == ANKAN) || (Fuuro[i] == MINKAN) ||
                     (Fuuro[i] == KAKAN)) &&
                    Fuuro[i + 1] == tmp)
                    dora++;
            }
        ...
        for (i = 6; i <= 9; i++) {
            if (Dora[i]) {
                ...
                for (int j = 0; Fuuro[j] && j <= 12; j += 4) {
                    if (((Fuuro[j] == ANKAN) || (Fuuro[j] == MINKAN) ||
                         (Fuuro[j] == KAKAN)) &&
                        Fuuro[j + 1] == tmp)
                        dora++;
                }
    ...
    if (uradora) {
        ...
        if (uradora >= 10)
            YakuNameTable[URADORA] += (char)(uradora / 10) + 48;
    ...
    if (dora) {
        YakuNameTable[DORA] += " ";
        if (dora >= 10)
            YakuNameTable[DORA] += (char)(dora / 10) + 48;
...
bool MJScore::Is_Pinfu() {
    ...
    for (int i = 0; i < 10; i += 2) {
        if (kiriwake[i] == TOITU) {
            if (kiriwake[i + 1] == Jikaze || kiriwake[i + 1] == Bakaze ||
                kiriwake[i + 1] >= 35)
                return false;
...
bool MJScore::Is_Ittuu() {
    ...
    for (int i = 0; i < 10; i += 2) {
        if (kiriwake[i] == SYUNTU || kiriwake[i] == CHII) {
            if (kiriwake[i + 1] == 1)
                chk[0] = 1;
            else if (kiriwake[i + 1] == 4)
                chk[1] = 1;
            else if (kiriwake[i + 1] == 7)
                chk[2] = 1;
            else if (kiriwake[i + 1] == 11)
                chk[3] = 1;
            else if (kiriwake[i + 1] == 14)
                chk[4] = 1;
            else if (kiriwake[i + 1] == 17)
                chk[5] = 1;
            else if (kiriwake[i + 1] == 21)
                chk[6] = 1;
            else if (kiriwake[i + 1] == 24)
                chk[7] = 1;
            else if (kiriwake[i + 1] == 27)
                chk[8] = 1;
        }
    }
...
bool MJScore::Is_Tyanta() {
    ...
    for (int i = 0; i < 10; i += 2) {
        ...
        //コーツ
        else if (kiriwake[i] == ANKO || kiriwake[i] == ANKAN ||
                 kiriwake[i] == MINKAN || kiriwake[i] == KAKAN ||
                 kiriwake[i] == ANKO || kiriwake[i] == PON) {
            if (!(kiriwake[i + 1] % 10 == 1 || kiriwake[i + 1] % 10 == 9 ||
                  kiriwake[i + 1] > 30))
                return false;
            if (kiriwake[i + 1] > 30)
...
bool MJScore::Is_Sansyokudoukou() {
    ...
    memset(chk, 0, sizeof(chk));
    for (int i = 0; i < 10; i += 2) {
        if (kiriwake[i] == PON || kiriwake[i] == ANKO || kiriwake[i] == ANKAN ||
            kiriwake[i] == KAKAN || kiriwake[i] == MINKAN ||
            kiriwake[i] == AKA_PON1 || kiriwake[i] == AKA_PON2) {
            chk[kiriwake[i + 1]]++;
        }
    }
    for (int i = 1; i < 10; i++) {
