...
public:
    ...
    //赤牌有り
    void Avail_Akahai(bool arg) { Akahai = arg; }
    
    //クイタン有り
    void Avail_Kuitan(bool arg) { Kuitan = arg; }
    
    //満貫切り上げ有り
    void Avail_ManganKiriage(bool arg) { ManganKiriage = arg; }
    
    //場千五有り
    void Avail_Ba1500(bool arg) { Ba1500 = arg; }
    
    //国士13面待ちダブル役満有り
    void Avail_DoubleKokushi13(bool arg) { DoubleKokushi13 = arg; }
    
    //九連宝燈9面待ちダブル役満有り
    void Avail_DoubleTyuuren9(bool arg) { DoubleTyuuren9 = arg; }
    
    //大四喜ダブル役満有り
    void Avail_DoubleDaisuusii(bool arg) { DoubleDaisuusii = arg; }
    
    //四暗刻単騎ダブル役満有り
    void Avail_DoubleSuttan(bool arg) { DoubleSuttan = arg; }
    
    //***** 手牌等入力
    
    //非フーロ牌入力
    void Set_Tehai(int t[]) { memcpy(Tehai, t, sizeof(int) * 41); }
    
    //フーロ牌入力
    void Set_Fuuro(int f[]) { memcpy(Fuuro, f, sizeof(int) * 20); }
    
    //ドラ牌入力
    void Set_Dora(int d[]) { memcpy(Dora, d, sizeof(int) * 9); }
    
    //自風
    void Set_Jikaze(int arg) { Jikaze = arg; }
    
    //場風
    void Set_Bakaze(int arg) { Bakaze = arg; }
    
    //あがり牌
    void Set_Agarihai(int arg) { Agarihai = arg; }
    
    //ツモあがり
    void Set_Tumoagari(int arg) { Tumo = arg; }
    
    // N本場
    void Set_Honba(int arg) { Honba = arg; }
    
    //供託リーチ
    void Set_KyoutakuRIichi(int arg) { KyoutakuRiichi = arg; }
    
    //ドラ数外部入力 v1.03
    void Set_Dorasuu(int arg) { inputteddora = arg; }
    
    //***** 以下の偶然役は成立時のみフラグをセットします
    
    //リーチ
    void Is_Riichi(bool arg) { Riichi = arg; }
    
    //天和
    void Is_Tenhou(bool arg) { Tenhou = arg; }
    
    //地和
    void Is_Tiihou(bool arg) { Tiihou = arg; }
    
    //ダブルリーチ
    void Is_DoubleRiichi(bool arg) { DoubleRiichi = arg; }
    
    //一発
    void Is_Ippatu(bool arg) { Ippatu = arg; }
    
    //チャンカン
    void Is_Tyankan(bool arg) { Tyankan = arg; }
    
    //リンシャン
    void Is_Rinsyan(bool arg) { Rinsyan = arg; }
    
    //流し満貫
    void Is_NagashiMangan(bool arg) { NagashiMangan = arg; }
    
    //ハイテイ
    void Is_Haitei(bool arg) { Haitei = arg; }
    ...
    std::string Get_ErrorCodeString() {
        ...
        if (ErrorCode == MIKEISAN)
            r = "未計算";
        if (ErrorCode == SEIJYOU)
            r = "正常終了";
        if (ErrorCode == HAIGA14MAIJYANAI)
            r = "牌が14枚じゃない";
        if (ErrorCode == AGARIHAIGAOKASII)
            r = "あがり牌がおかしい";
        if (ErrorCode == AGATTEINAI)
            r = "あがっていない";
        if (ErrorCode == BAKAZEKAJIKAZEGANAI)
            r = "場風か自風がない";
        if (ErrorCode == NAKIRIICHI)
            r = "鳴きリーチ";
        if (ErrorCode == YAKUNASI)
            r = "役なし";
        return r;
    }
    
    //親ロン取得
    int Get_OyaRon() { return OyaRon; }
    
    //親ツモ取得
    int Get_OyaTumo() { return OyaTumo; }
    
    //子ロン取得
    int Get_KoRon() { return KoRon; }
    
    //子ツモ(親)取得
    int Get_KoTumoOya() { return KoTumoOya; }
    
    //子ツモ(子)取得
    int Get_KoTumoKo() { return KoTumoKo; }
    
    //符取得
    int Get_Fu() { return Fu; }
    
    //飜数取得
    int Get_Fan() { return Fan; }
    
    //最終的に親が貰う点数(ロン)
    int Get_TokutenOya() { return TokutenOya; }
    
    //最終的に子が貰う点数(ロン)
    int Get_TokutenKo() { return TokutenKo; }
    
    //最終的に親が貰う点数(ツモ)
    int Get_TokutenOyaTumo() { return TokutenOyaTumo; }
    
    //最終的に子が貰う点数(ツモ)
    int Get_TokutenKoTumo() { return TokutenKoTumo; }
    
    //成立した役テーブル取得
    void Get_Yakutable(bool t[]) { memcpy(t, Yaku, sizeof(bool) * 60); }
    
    //成立した役名一覧
    void Get_Yakuname(string[]);
    
    //ドラ数(赤除く)
    int Get_Dorasuu() { return dora; }
    
    //裏ドラ数
    int Get_Uradorasuu() { return uradora; }
    
    //赤ドラ数
    int Get_Akadorasuu() { return akadora; }
    
    //マンガンかどうか
    bool Get_Ismangan() { return IsMangan; }
